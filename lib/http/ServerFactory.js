...
class ServerFactory {
  ...
  **/
  createServer() {
    ...
    // register config to $injector
    requestHandler.configInjector(function($injector) {
      $injector.put("$config", function() {
        return config;
      });
    ...
    for(var m = 0;m < services.length;m++) {
      requestHandler.registerService(services[m].name, services[m].factory);

      console.log(services[m].name + " is loaded");
    }
